...
public class StreamMain {
	...
	public static void main(String[] args) {
		...
		// 최종 작업만 수행해서 데이터 출력
		list.stream().forEach(e -> {System.out.println(e);}); // e는 매개변수

		// 데이터 3개만 출력
		list.stream().limit(3).forEach(e -> {System.out.println(e);});

		// 데이터 정렬 후 출력
		list.stream().sorted().forEach(e -> {System.out.println(e);}); // 오름차순 정렬

		// 내림차순 정렬
		// sorted 메소드에 내림차순 정렬을 위한 Comparator 인터페이스를 구현한
		// 클래스의 객체를 설정하면 됨
		// Comparator 인터페이스는 매개변수가 2개이고 정수를 리턴하는 메소드 1개만 존재
		...
		// 데이터를 정수로 변환해서 정렬
		// 중간 처리 메소드 중에는 Int로 리턴해주는 mapToInt 라는 메소드가 존재하고
		// 이 메소드를 사용할 때는 변환에 사용하는 메소드를 설정만 해주면 됨
		// 클래스이름::메소드이름
		...
		// isPresent 라는 메소드를 이용해서 null 여부를 판단하고
		// get이라는 메소드로 데이터를 가져옴
		OptionalDouble result = list.stream()
		.mapToInt(Integer::parseInt)
		.filter(o -> o % 2 != 0)
		.average();
		// min, max, average는 optional로 return
